// From https://gimplearn.net/dmc_color_picker.php released under MIT license with permission.
...
function rectangle_blend_dots(x, y, bdmc, blend) {
  ...
  for (var i = 0; i < height + 1; i++) { //i in range(0,height+1):
    for (var j = 0; j < width + 1; j++) { //in range(0,width+1):
      if ((Math.ceil(i / blend) + Math.ceil(j / blend)) % 2 == 0) {
        color = DMC[bdmc[1]][3];
      } else {
        color = DMC[bdmc[2]][3];
      }
...
function mydraw() {
  ...
  for (var i = 0; i < 28; i++) { // in range(0,28):
    if (blend == 0) {
      ...
      outText(DMC[i][0], 310, 47 + i * 20);
      outText(DMC[i][1], 370, 47 + i * 20);
    }
    if (blend > 0) {
      rectangle_blend_dots(200, 35 + i * 20, BDMC[i], blend);
      outText(DMC[BDMC[i][1]][0] + "," + DMC[BDMC[i][2]][0], 310, 47 + i * 20);
      outText(DMC[BDMC[i][1]][1] + "," + DMC[BDMC[i][2]][1], 400, 47 + i * 20);
...
function sort_closest_color_perceptive() {
  ...
  var array = $.map(DMC, function(value, index) {
    return [value];
  });
  DMC = array;
...
function find_blend(b) {
  document.body.style.cursor = 'progress';

  blend = b;
  BDMC = [];
  for (var j = 0; j < 28; j++) { //j in range(0,len(DMC)-1):
    for (var k = j; k < 29; k++) { //k in range(j,len(DMC)):
      newblend = ["Blend", j, k, [
        Math.round((DMC[j][3][0] + DMC[k][3][0]) / 2.0),
        Math.round((DMC[j][3][1] + DMC[k][3][1]) / 2.0),
        Math.round((DMC[j][3][2] + DMC[k][3][2]) / 2.0)
      ], 0]
